...
def _seed_runs(root: Path):
    for variant in ("legacy_v4", "refactor_no_dossier", "refactor_full"):
...
def test_next_returns_anonymized_pair(tmp_path):
    ...
    assert {r["a"]["variant"], r["b"]["variant"]} <= {
        "legacy_v4", "refactor_no_dossier", "refactor_full",
    }
    assert r["a"]["variant"] != r["b"]["variant"]
...
def test_judge_appends_jsonl(tmp_path):
    client = TestClient(_app(tmp_path))
    from echuu.eval import serve
    payload = {"fixture_id": "c1", "variant_a": "legacy_v4",
               "variant_b": "refactor_no_dossier", "winner": "a"}
...
def test_audio_missing_returns_404(tmp_path):
    ...
    (tmp_path / "runs" / "c1" / "legacy_v4" / "audio.wav").unlink()
    client = TestClient(app)
    assert client.get("/audio/c1/legacy_v4").status_code == 404
    assert client.get("/audio/c1/bogus_variant").status_code == 404


def test_next_done_after_all_judged(tmp_path):
    # c1 has 3 variants -> 3 pairs. Judging all
